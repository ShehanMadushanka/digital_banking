...
class AppValidator {
  ...
  bool advancedNicValidation(String nic) {
    ...
    //OLD NIC *****
    if (nic.length == 10) {
      var nicPart1 = nic.substring(nic.length - 1);
      var nicPart2 = nic.substring(0, nic.length - 1);

      // print("nic_part1 $nicPart1");
      // print("nic_part2 $nicPart2");

      final List<int> numbers = [];

      for (int i = 0; i < nicPart2.length; i++) {
        numbers.add(int.parse(nicPart2[i]));
      }
      ...
      for (int i = 0; i < numbers.length; i++) {
        values.add(numbers[i] * kConstantOldNIC[i]);
      }
//     print(values);

      final int sum = values.reduce((value, element) => value + element);
      // print(sum);

      int modulesVal = sum % 11;
      // print("modulesVal $modulesVal");

      if (modulesVal <= 1) {
        // modulesVal <= 1
        final int lastDigit = int.parse(nicPart2[nicPart2.length - 1]);
        ...
        // modulesVal > 1
        final int remianValue = 11 - modulesVal;
        final int lastDigit = int.parse(nicPart2[nicPart2.length - 1]);

        // print("remianValue $remianValue");
        // print("lastDigit $lastDigit");

        if (lastDigit == remianValue) {
          return true;
        } else {
          return false;
        }
      ...
      for (int i = 0; i < numbers.length; i++) {
        values.add(numbers[i] * kConstantNewNIC[i]);
      }
//     print(values);
      ...
      final int modulesVal = sum % 11;

      if (modulesVal <= 1) {
        // modulesVal <= 1
        final int lastDigit = int.parse(nic[nic.length - 1]);
  ...
  bool nicDobValidate(String nic, String dateOfBirth)
  {
    ...
    try {
      ...
      if (dateResponse.year == dob.year && dateResponse.month == dob.month && dateResponse.day == dob.day) {
        isValid = true;
      }
  ...
  DateResponse getDateResponse(String nic) {
    ...
    final List<int> monthDaysList = [31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
